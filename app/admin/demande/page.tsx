...
import { CalendarDays, Check, ClipboardClock, Hourglass, X } from "lucide-react"
...
const demandeItem = [
  ...
  { titre : "demande de Certificat " , date : '16/02/2026' ,status : "En attent"  , nom : "Bryan"} ,
]

const billetItem = [
  {nom : "Bryan" ,class : "2nd", date : '14/05/26' , motif : "maladie"},
  {nom : "Jean" ,class : '2nd', date : '14/05/26' , motif : "maladie"},
  {nom : "Paule" ,class :"6 eme", date : '14/05/26' , motif : "maladie"},
  {nom : "Jerie" , date : '14/05/26' , motif : "maladie"},
  {nom : "Eric" , date : '14/05/26' , motif : "maladie"},
]

const reunionItem = [
  {nom : "Bryan" , date : '14/05/26' , motif : "maladie"},
  {nom : "Jean" , date : '14/05/26' , motif : "maladie"},
  {nom : "Paule" , date : '14/05/26' , motif : "maladie"},
  {nom : "Jerie" , date : '14/05/26' , motif : "maladie"},
  {nom : "Eric" , date : '14/05/26' , motif : "maladie"},
]
const page = () => {
  return (
    <div className="min-h-screen p-5 space-y-4">
      <div className="grid grid-cols-4 gap-2 ">
          {statItems.map((item) =>(
            <Card key={item.titre} className="shadow-none rounded-sm hover:scale-y-105 hover:shadow-sm transition-all duration-300 hover:bg-sky-50">
              <CardHeader>
        ...
        <main className="max-h-2xl h-full  flex w-full gap-3">
          <Card className="w-full   rounded-sm  shadow-none p-5 max-h-96 ">
            <CardHeader className=" flex items-center gap-3">
              ...
              <Hourglass />
              <p className="font-semibold">Demande Obtenue</p>
            </CardHeader>
          ...
          <Card className="container w-2/3  h-full max-h-96 shadow-none">
            <CardHeader>
            ...
            <CardContent className="flex flex-col gap-2 overflow-auto ">
              ...
              >
                <div className="flex items-center">
                  <div>
                    <span className="flex gap-2 items-center"><p>{item.nom}</p> <Badge variant='outline'>{item.class}</Badge></span>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-400 leading-tight">
                      {item.motif}
                    </p>
                    <p className="text-xs text-slate-400 mt-0.5">
        ...
        <Card className="border h-2/3 max-h-72 rounded-sm">
          <CardHeader>
          ...
          <CardContent className="flex flex-col gap-2 overflow-auto ">
              ...
              >
                <div className="flex items-center">
                  <div>
                    <span className="flex gap-2 items-center">{item.nom} le {item.date}</span>
                    <p className="text-sm font-medium text-slate-700 dark:text-slate-400 leading-tight">
                      {item.motif}
                    </p>
